...
1. `health-report.html` - 主頁面與表單（趣味小工具定位、免責聲明、狗狗體型說明與品種範例）
2. `health-guidelines.json` - 健康指引資料庫
3. `health-calculator.js` - 核心計算邏輯
4. `health-report-generator.js` - Canvas 圖片生成器（1080×1700、健康提醒換行、頁尾不重疊、QR 連結官網）
5. `health-report-ui.js` - 表單互動控制（倉鼠體重預設公克、狗狗／倉鼠專用區塊）
...
負責將計算結果繪製成 1080×1440 的 PNG 圖片，使用純 Canvas API 完成所有繪圖。
...
class PetHealthReportGenerator {
    constructor(reportData) {
        ...
        this.canvas.height = 1700;  // 健康提醒卡片與頁尾不重疊
    }
}
```
...
async generate() {
    ...
    this.drawBodyConditionCard();  // 體況評估卡片（如有體重）
    this.drawNutritionCard();      // 飲食建議卡片
    this.drawHealthTipsCard();     // 健康提醒卡片
    await this.drawFooter();       // 底部：門市資訊 + QR Code
    return this.canvas;
}
```
...
│ └─────────────────────────────────┘ │
├─────────────────────────────────────┤ 1100
│ 💊 健康提醒卡片（4 條、自動換行）    │ 高度 360
├─────────────────────────────────────┤ 1460
│ 📍 門市資訊 + QR Code（y=1480 起）   │
│ 免責：不能取代專業獸醫，健康疑慮請諮詢獸醫或儘速就醫 │
└─────────────────────────────────────┘ 1700
```
...
3. ✅ 切換年齡輸入方式（出生日期/直接輸入）
4. ✅ 提交表單 → 顯示載入動畫 → 生成報告
5. ✅ Canvas 正確顯示所有資訊
6. ✅ 分享功能（手機測試 Web Share API）
7. ✅ 下載 PNG 檔案
8. ✅ 重新測試按鈕正常運作
