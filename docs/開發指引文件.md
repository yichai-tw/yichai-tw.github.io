...
4. **health-report-generator.js** - Canvas 圖片生成器（1080×1440 直式 3:4、卡片間 cardGap 避免重疊、頁尾依 currentY 繪製、體型 5 級愛心、飲食區間與疾病提醒、QR 連結官網）
...
```javascript
async generate() {
    await this.loadAssets();      // 載入字體、Logo、圖示
    this.drawBackground();         // 繪製漸層背景
    this.drawHeader();             // 頂部：Logo + 標題
    this.drawAgeCard();            // 人類年齡卡片
    this.drawStageCard();          // 生命階段卡片
    this.drawBodyConditionCard();  // 體型與活動參考（5 級愛心、飼主稱讚）
    this.drawNutritionCard();      // 飲食建議（熱量/乾糧/飲水區間、勾選疾病飲食提醒）
    this.drawHealthTipsCard();     // 健康提醒（性別關注 + 疾病 + 生命階段，自動換行）
    await this.drawFooter(currentY);  // 底部：依卡片結束位置繪製，避免重疊
    return this.canvas;
}
```
...
│ 📋 生命階段卡片                      │ 高度 152
├─────────────────────────────────────┤ 472
│ ⚖️ 體型與活動參考（5 級愛心 ♥♡、飼主稱讚） │ 高度 200
├─────────────────────────────────────┤ 672
│ 🍲 飲食建議（熱量/乾糧/飲水區間；勾選疾病飲食提醒；註記已依品種/年齡/體重/體型/性別計算） │ 高度動態 212~268
├─────────────────────────────────────┤
│ 💊 健康提醒（性別／結紮關注 + 疾病 + 生命階段，最多 4 條、自動換行） │ 高度 260
├─────────────────────────────────────┤
│ 卡片間 cardGap（12px），頁尾 drawFooter(currentY) 不重疊          │
│ 📍 門市資訊 + QR Code、免責聲明（距畫布底 20px）                   │
└─────────────────────────────────────┘ 1440
```
...
5. ✅ 提交表單 → 顯示載入動畫 → 生成報告
6. ✅ Canvas 正確顯示：人類年齡、生命階段、體型 5 級愛心與稱讚、飲食區間、健康提醒（性別／結紮關注＋勾選疾病）；卡片間有間距、頁尾不重疊
